...
const C = {
  base: "#5d6066", baseSide: "#34363c", road: "#34363b", line: "#e9eaec",
  cream: "#e6e1d4", white: "#eef0f2", paleBlue: "#aab6c6", slate: "#2b3656", beige: "#d8c8a6",
  glass: "#27303f", glassBlue: "#3c4d6b", roof: "#8a8d94", entrance: "#9a9da4",
  leaf: "#5d8a46", leaf2: "#4f7a3c", trunk: "#5a4632", grass: "#5d8a46", path: "#8a8d94", skin: "#caa988",
};
const TOPS = ["#d98a3a", "#3f6fb0", "#7a8090", "#b85a4a", "#4f7a4a", "#9a8f86"];
...
function makeGridTex() {
  ...
  x.fillStyle = "#2a3340"; x.fillRect(6, 6, 20, 20);
...
function makeStripTex() {
  ...
  x.fillStyle = "#2a3340"; x.fillRect(8, 0, 16, 8);
...
function bShop(b: Bld) {
  ...
  const awns = ["#b5382f", "#3f7a3a", "#2f6fb0", "#c08a2e"];
  const out: React.ReactNode[] = [];
  for (let u = 0; u < n; u++) {
    const cx = (u - (n - 1) / 2) * uw, col = pick(bodyCols), awn = pick(awns), fz = dep / 2, key = u * 20;
    out.push(box([uw - 0.12, h, dep], [cx, h / 2, 0], col, key));
    out.push(box([uw * 0.86, 0.6, 0.12], [cx, h * 0.78, fz + 0.05], "#2b3656", key + 1));
    out.push(box([uw * 0.3, 0.55, uw * 0.3], [cx + uw * 0.16, h + 0.28, -dep * 0.16], C.roof, key + 2));
    const sn = 4;
    for (let k = 0; k < sn; k++) { const w = (uw * 0.86) / sn; out.push(box([w, 0.24, 0.9], [cx + (k - (sn - 1) / 2) * w, h * 0.6, fz + 0.45], k % 2 ? "#ece9df" : awn, key + 3 + k)); }
...
function Auto() { return (<group>{box([2.6, 1.2, 1.8], [0, 0.95, 0], C.leaf)}{box([2.6, 0.9, 1.85], [0, 1.55, 0], "#e0b020")}{box([2.7, 0.5, 1.9], [0, 2.15, 0], "#1a1a1a")}{box([0.5, 0.2, 0.2], [1.35, 0.75, 0.45], C.line)}{box([0.55, 0.55, 0.3], [1.0, 0.27, 0], TYRE, 70)}{box([0.55, 0.55, 0.3], [-0.85, 0.27, 0.85], TYRE, 71)}{box([0.55, 0.55, 0.3], [-0.85, 0.27, -0.85], TYRE, 72)}</group>); }
...
const VCOLS = [C.white, "#b85a4a", "#b9bcc4", "#3a3d45", "#cfd2da", "#3f6fb0", "#2f8f6a", "#e0a23a", "#d24b3e", "#7a5bb0", "#e8e8ea"];
const BUS_COLS = ["#2f8f4a", "#3f6fb0"];
...
function TrafficLight({ x, z }: { x: number; z: number }) {
  ...
  return (
    <group position={[x, 0, z]}>
      ...
      <mesh position={[0, 4.85, 0.28]}><boxGeometry args={[0.26, 0.26, 0.18]} /><meshStandardMaterial ref={r} color="#ff3b3b" emissive="#ff3b3b" emissiveIntensity={0.05} /></mesh>
      <mesh position={[0, 4.4, 0.28]}><boxGeometry args={[0.26, 0.26, 0.18]} /><meshStandardMaterial ref={a} color="#ffb020" emissive="#ffb020" emissiveIntensity={0.05} /></mesh>
      <mesh position={[0, 3.95, 0.28]}><boxGeometry args={[0.26, 0.26, 0.18]} /><meshStandardMaterial ref={g} color="#37d67a" emissive="#37d67a" emissiveIntensity={0.05} /></mesh>
      <mesh position={[0, 4.85, -0.28]}><boxGeometry args={[0.26, 0.26, 0.18]} /><meshStandardMaterial ref={r2} color="#ff3b3b" emissive="#ff3b3b" emissiveIntensity={0.05} /></mesh>
      <mesh position={[0, 4.4, -0.28]}><boxGeometry args={[0.26, 0.26, 0.18]} /><meshStandardMaterial ref={a2} color="#ffb020" emissive="#ffb020" emissiveIntensity={0.05} /></mesh>
      <mesh position={[0, 3.95, -0.28]}><boxGeometry args={[0.26, 0.26, 0.18]} /><meshStandardMaterial ref={g2} color="#37d67a" emissive="#37d67a" emissiveIntensity={0.05} /></mesh>
    </group>
  );
}
  ...
  return (
    <group>
      ...
      <InstGroup objects={data.streetLights} parts={[
        ...
        { size: [0.5, 0.35, 0.5], off: [1.1, 3.75, 0], color: "#ffcf8a", emissive: 1.3 },
      ]} />
  ...
  return (
    ...
    <Canvas
      ...
      onCreated={({ scene, camera }) => {
        ...
        scene.fog = new THREE.Fog("#111111", 360, 1100);
        camera.lookAt(0, 10, -70);
      }}
    >
      <ambientLight intensity={0.72} color="#aab0c0" />
      <directionalLight
        position={[240, 340, 180]} intensity={1.55} color="#fff7ec" castShadow
        ...
        shadow-bias={-0.0004}
      />
      <directionalLight position={[-200, 130, -140]} intensity={0.4} color="#9aa0c0" />
      <City />
    </Canvas>
    </div>
  );
